...
sql="
select key, value, items.album_id, items.album from item_attributes
join items on entity_id = items.id order by key, value*1.0, value, items.album_id;
"

echo $sql | sqlite3 -separator ' ' ~/beet_musiclibrary.blb > key_diversity.1
